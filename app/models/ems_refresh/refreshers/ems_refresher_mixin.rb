  ...
  module Refreshers
    module EmsRefresherMixin
      ...
      def preprocess_targets
        ...
        @targets_by_ems_id.each do |ems_id, targets|
          ...
          if ems_in_list
            ...
            targets.clear << ems
          elsif !ems_in_list && targets.length >= @full_refresh_threshold
      ...
      def refresh_targets_for_ems(ems, targets)
        ...
        targets_with_inventory, _ = Benchmark.realtime_block(:get_inventory_for_targets) do
          get_inventory_for_targets(ems, targets)
        end
        ...
        until targets_with_inventory.empty?
          ...
          hashes, _ = Benchmark.realtime_block(:parse_targeted_inventory) do
            parse_targeted_inventory(ems, inventory, target)
          end
      ...
      def get_inventory_for_targets(ems, targets)
        ...
        targets.collect { |target| [target, []] }
      end

      def parse_targeted_inventory(ems, inventory, target)
        ...
        hashes, _ = Benchmark.realtime_block(:parse_legacy_inventory) { parse_legacy_inventory(ems, target) }
        hashes
      end

      def parse_legacy_inventory(ems, target)
        # TODO: remove this method after all refreshers support retrieving
        # inventory separate from parsing
        parse_inventory(ems, target)
      end
